        ...
        <q-card class="n-card q-mr-lg">
          <q-list bordered separator>
            ...
            <q-item
              ...
              @click="editDraft(draft)"><q-item-section> {{ draft.title }}</q-item-section></q-item>
          </q-list>
        </q-card>
      </template>
        ...
        <edit-post v-model="draftedPost">
          <template v-slot:actions>
            <q-btn
              label="publish"
              color="positive"
              :disable="!isDirty || !draftedPost?.title || !draftedPost.markdown
        "
              @click="onPublishPost"></q-btn>
            <q-space />
            <q-btn
              label="un-publish"
              color="negative"
              @click="onUnpublishPost"></q-btn>
          </template>
        </edit-post>
      </template>
    </q-splitter>
...
<script setup lang="ts">
  ...
  const onPublishPost = () => {
    q.dialog({
      ...
      if (!isDirty.value)
        throw new Error('Draft is unchanged! Aborting.');
