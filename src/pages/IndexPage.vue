...
<script setup lang="ts">
  ...
  const log = useLogger('IndexPage.vue');
  ...
  defineOptions({
    // TODO: need to rethink my prefetch call since i can only return one promise
    preFetch: ({ store }) => {
      const plog = useLogger('PREFETCH IndexPage.vue');
      plog.debug('running prefetch');
      const ps = usePostStore(store);
      return ps.getRandomDraft();
    },
  });
</script>
