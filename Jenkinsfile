    ...
    stages {
        stage('Prepare') {
            steps {
                ...
                withCredentials([file(credentialsId: 'nb-blog-ui-env-file', variable: 'ENV_FILE')]) {
        ...
        stage('Build') {
            steps {
                sh 'docker build -t novabyte-ui:latest .'
                sh 'docker save -o nb-ui_docker-image.tar novabyte-ui:latest'
                sh 'rm -f nb-ui_docker-image.tar.xz'
                sh 'xz -T0 -9 nb-ui_docker-image.tar'
            }
        }
